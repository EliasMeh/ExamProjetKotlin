...
import com.example.examprojet.model.InflationRepository
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext

class InflationViewModel : ViewModel() {
    private val tauxinflationRepo = InflationRepository()
        ...
        viewModelScope.launch {
            try {
                withContext(Dispatchers.IO) {
                    val exchangeRates = tauxinflationRepo.getInflationRate()
                    ...
                    if (exchangeRates != null) {
                        val sortedExchangeRates = exchangeRates.sortedBy { it.time_period_end }
                        sortedExchangeRates.forEach { observation ->
                            if (observation.obs_value != 0.0) {
                                valueMutableList.add(observation.obs_value)
                                timePeriodMutableList.add(observation.time_period_end)
                            }
                        }
                    } else {
